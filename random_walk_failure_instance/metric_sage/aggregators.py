    ...
    def forward(self, nodes, neigh_nodes, metric, num_sample=10, is_node_train_index=True):
        ...
        try:
            ...
            if is_node_train_index:
                ...
                if self.cuda:
                    embed_matrix = self.fc(torch.from_numpy(np.float32(self.metric.loc[unique_nodes_list].values)).cuda()
                                      .view(len(unique_nodes_list), self.feature_num))
                else:
                    embed_matrix = self.fc(torch.from_numpy(np.float32(self.metric.loc[unique_nodes_list].values))
                                      .view(len(unique_nodes_list), self.feature_num))
