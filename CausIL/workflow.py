...
def workflow(config: RandomWalkFailureInstanceConfig):
    ...
    with profile("random_walk_main", report_printer=lambda _: logger.info(f"\n{_}")) as profiler:
        folder = "/Users/zhuyuhan/Documents/391-WHU/experiment/researchProject/MicroIRC/data/data2/A1"
        minute = 10
        # time_data
        ...
        #     t = Time(begin_timestamp, end_timestamp, root_cause, root_cause_level, failure_type, lb)
        #     time_list.append(t)
        # graphsage = trainGraphSage(time_list, class_num, None)
        nums = []
        acc_count = 0
        acc = 0
        for fid in tqdm(base.test_failure_ids):
            ...
            acc_temp = my_acc(failure_instance_scores, [root_cause])
            if acc_temp > 0:
                acc_count += 1
                acc += acc_temp
...
def trainGraphSage(time_list, class_num, label_file ,train = False):
    # build svc call
    ...
    metric_source_data = pd.read_csv('/Users/zhuyuhan/Documents/391-WHU/experiment/researchProject/MicroIRC/data/data2/D/metric.norm.csv')
    metric_source_data = metric_source_data.fillna(0)
    ...
    return run_RCA(node_num, 159, data, time_data, time_list, data, None, class_num, label_file ,train, False)
